...
class RedisTokenRepository(
        private val template: RedisTemplate<String, RefreshToken>
) : TokenRepository {
    ...
    override fun setRefreshToken(refreshToken: RefreshToken) {
        template
                .opsForValue()
                .set(
                        getKey(refreshToken.userId),
                        refreshToken,
                        AuthConstants.REFRESH_TOKEN_TTL.toJavaDuration(),
                )
    }
    ...
    override fun getRefreshToken(userId: Long): RefreshToken? =
            template.opsForValue().get(getKey(userId))
