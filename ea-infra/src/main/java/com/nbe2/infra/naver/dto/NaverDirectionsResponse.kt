...
data class NaverDirectionsResponse(
    ...
    data class Route(@JsonProperty("traoptimal") val traoptimal: List<Traoptimal>)
    ...
    data class Goal(@JsonProperty("location") val location: List<Double>, @JsonProperty("dir") val dir: Int)
    ...
    companion object {
        ...
        private fun convertRoute(route: Route): EmergencyRoomDirectionsInfo.Route {
            val traoptimalList = route.traoptimal.map { convertTraoptimal(it) }
            return EmergencyRoomDirectionsInfo.Route(traoptimalList)
        }

        private fun convertTraoptimal(traoptimal: Traoptimal): EmergencyRoomDirectionsInfo.Traoptimal {
        ...
        private fun convertSummary(summary: Summary): EmergencyRoomDirectionsInfo.Summary {
            return summary.let {
                EmergencyRoomDirectionsInfo.Summary(
                    start = EmergencyRoomDirectionsInfo.Start(it.start.location),
                    goal = EmergencyRoomDirectionsInfo.Goal(it.goal.location, it.goal.dir),
        ...
        private fun convertSection(section: Section): EmergencyRoomDirectionsInfo.Section {
        ...
        private fun convertGuide(guide: Guide): EmergencyRoomDirectionsInfo.Guide {
