...
@Component
class CustomSecurityFilter(private val jwtProvider: JwtProvider) :
    ...
    override fun doFilterInternal(
        ...
        try {
            ...
            val tokenUserPrincipal: UserPrincipal =
                    jwtProvider.getTokenUserPrincipal(jwtToken)
    ...
    private fun convertorGrantedAuthority(
            role: String
    ...
    private fun reversToken(httpServletRequest: HttpServletRequest): String {
        val bearerToken: String =
                httpServletRequest.getHeader(EAConstants.AUTHORIZATION_HEADER)
        if (
                StringUtils.hasText(bearerToken) &&
                        bearerToken.startsWith(EAConstants.BEARER)
        ) {
