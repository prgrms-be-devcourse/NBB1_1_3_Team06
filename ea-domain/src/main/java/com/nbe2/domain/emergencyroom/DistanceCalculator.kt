...
class DistanceCalculator(
    ...
    fun calculate(
        ...
        return realTimeEmergencyRoomInfos.stream()
            .map { roomInfo -> calculateDistance(currentCoordinate, roomInfo) }
            .sorted(Comparator.comparing(RealTimeEmergencyRoomWithDistance::distance))
            .toList()
    }

    fun calculateDistance(
        currentCoordinate: Coordinate, info: RealTimeEmergencyRoomInfo
    ): RealTimeEmergencyRoomWithDistance {
        val emergencyRoom = emergencyRoomReader.read(info.hpId)
        val distance = currentCoordinate.distanceTo(emergencyRoom.getCoordinate())
