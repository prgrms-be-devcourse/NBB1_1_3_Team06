...
class EmergencyRoomInitializer(
    ...
    fun init() {
        ...
        val emergencyRooms: List<EmergencyRoom> =
                removeDuplicatesEmergencyData(uniqueKeySet)
        if (emergencyRooms.isNotEmpty())
                emergencyRoomRepository.saveAll(emergencyRooms)
    }
    ...
    private fun removeDuplicatesEmergencyData(
            uniqueKeySet: Set<String>
    ): List<EmergencyRoom> {
        // nullable인 경우 처리
        return emergencyRoomClient
                .getEmergencyRoomInfoData()
                .map {
                    it.toEmergencyRoom()
                } // EmergencyRoomInfo를 EmergencyRoom으로 변환
                .filter { room ->
                    // 병원 이름, 경도, 위도를 연결해 고유 키 생성
                    val uniqueKey =
                            "${room.hospitalName}${room.location.x}${room.location.y}"
