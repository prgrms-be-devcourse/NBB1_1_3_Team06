...
import com.nbe3.domain.global.ID
...
class UserReaderTest : BehaviorSpec({
    ...
    Given("유효한 ID가 주어지는 경우") {
        ...
        every { userRepository.findById(any()) } returns Optional.of(expected)
    ...
    Given("회원이 존재하지 않는 경우") {
        ...
        When("ID로 사용자를 조회하면") {
            Then("예외가 발생한다.") {
                shouldThrow<UserNotFoundException> { userReader.read(userId) }
            }
        }
    }
    ...
    Given("존재하지 않는 이메일이 주어지는 경우") {
        ...
        When("이메일로 사용자를 조회하면") {
            Then("예외가 발생한다.") {
                shouldThrow<UserNotFoundException> { userReader.read(email) }
            }
        }
    }
    ...
    Given("승인 대기 상태의 사용자가 존재하지 않는 경우") {
        ...
        val emptyPage = PageImpl(emptyList<UserProfileWithLicense>(), pageable, 0)
    ...
    Given("승인 대기 상태의 사용자가 존재하는 경우") {
        ...
        val emptyPage = PageImpl(content, pageable, content.size.toLong())
    ...
    Given("승인 대기 상태의 사용자가 마지막 페이지에 있는 경우") {
        ...
        every {
            userRepository.findPageByApprovalStatus(any<ApprovalStatus>(), any())
        } returns page
