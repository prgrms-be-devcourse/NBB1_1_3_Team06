...
class NotificationServiceTest :
        BehaviorSpec({
            ...
            val notificationService =
                    NotificationService(notificationUpdater, notificationReader)
            ...
            Given("커서가 주어지는 경우") {
                ...
                every { notificationReader.getNextCursor(any(), any()) } returns
                        expected.nextCursor
                ...
                When("알림 기록을 조회하면") {
                    ...
                    Then("기록이 포함된 커서 결과를 볼 수 있다.") {
                        verify {
                            notificationUpdater.readAllUnreadNotifications(
                                    userId
                            )
                        }
            ...
            Given("알림 기록이 없는 경우") {
                ...
                val cursor = Cursor(Long.MAX_VALUE, 2)
                val expected =
                        CursorResult(emptyList<NotificationDetail>(), null)

                every { notificationReader.read(any(), any()) } returns
                        emptyList()
